...
class RequestHandler {
    constructor({ router, authenticationRepo, host, tokenType }) {
        ...
        this.verifyKey = () => {
            return (req, res, next) => {
                const response = this.authenticationRepo.verifyBearerToken(req.headers["authorization"], encryption_1.TokenType.emailVerification);
                if (response.status === false) {
                    return res.status(403).json({
                        status: response.status,
                        noPublicKey: true,
        ...
        this.router = router;
        this.host = host;
        this.tokenType = tokenType !== null && tokenType !== void 0 ? tokenType : encryption_1.TokenType.accessToken;
    }
    get listEndPoint() {
