...
class TradeRepository {
    ...
    constructor() {
        ...
        this.swapTokens = ({ tokenIn, tokenOut, amount, wallet }) => __awaiter(this, void 0, void 0, function* () {
            try {
                const web3Provider = new ethers_1.ethers.providers.JsonRpcProvider('https://goerli.infura.io/v3/40544a9882294178bf9427be38bea3a5');
                ...
                const routerAbi = yield this.getABI(V3SWAPROUTERADDRESS);
                ...
                const router = new ethers_1.ethers.Contract(V3SWAPROUTERADDRESS, routerAbi, provider);
                ...
                const amountIn = ethers_1.ethers.utils.parseEther(amount.toString()); // 1 DAI
                const amountOutMin = ethers_1.ethers.utils.parseEther('0'); // minimum amount of WETH to receive
                const path = [GOERLIADDRESS, WETHADDRESS]; // the swap path
                const to = wallet.address; // the recipient address
                const deadline = Math.floor(Date.now() / 1000) + 60 * 20; // 20 minutes from now
                const tokenAbi = yield this.getABI(tokenIn.address);
                const tokenContract = new ethers_1.ethers.Contract(GOERLIADDRESS, tokenAbi, provider);
                const tokenApproval = yield tokenContract.connect(signer).approve(V3SWAPROUTERADDRESS, this.fromReadableAmount(amount * 2, tokenIn.decimals).toString());
                ...
                const wallets = new ethers_1.ethers.Wallet(privateKey, provider);
                ...
                //   deadline
                // );
                const tx = yield router.connect(signer).swapExactTokensForETH(amountIn, amountOutMin, path, to, deadline, { gasLimit: ethers_1.ethers.utils.hexlify(100000), });
                console.log('tx', tx);
